    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
            <div class="p-6">
                ...
                <form
                    action="{{ route('reservations.process', ['type' => $type, 'id' => $item->homestay_id ?? $item->activity_id]) }}"
                    method="POST">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @if($type === 'homestays')
                            <div>
                                ...
                                <input type="date" name="check_in_date" id="check_in_date"
                                    class="w-full px-3 py-2 border rounded-lg" required min="{{ now()->format('Y-m-d') }}">
                            </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="guests" id="guests" class="w-full px-3 py-2 border rounded-lg"
                        ...
                        <div class="col-span-2">
                            <label class="block text-gray-700 font-bold mb-2" for="special_requests">
                                Special Requests (Optional)
                            </label>
                            <textarea name="special_requests" id="special_requests"
                                class="w-full px-3 py-2 border rounded-lg" rows="4"></textarea>
                        </div>
                    </div>

                    <div class="mt-6">
                        <button type="submit"
                            class="w-full bg-[#C1FA8F] hover:bg-[#AFDF84] text-black font-bold py-3 px-4 rounded-lg transition duration-300">
                            Confirm Reservation
                        </button>
                    </div>
                </form>
